 ...
 */
function parse_php_tag($element, $parentName, $level)
{
    ...
    if (!empty ($children))
    {
        $response = parseTemplateRecursive($children, $level + 1);
    }
    else {
 ...
 * @return string $out
 */
function parse_math_tag(simpleXMLElement $element, $parentName, $level)
{
 ...
 * @return array|string
 */
function parse_google_tag($element, $parentName, $level)
{
 ...
 * @return string
 */
function parse_wiki_tag($element, $parentName, $level)
{
    global $debugemail;
