...
function unset_all_bad_pattern_matches($allrows, $lookingfor)
{
    ...
    $current_thatpattern = (isset ($_SESSION['convoArr']['that'][1][1])) ? $_SESSION['convoArr']['that'][1][1] : '';
    ...
    $default_pattern = $_SESSION['convoArr']['conversation']['default_aiml_pattern'];
    ...
    foreach ($allrows as $all => $subrow)
    {
        ...
        if (stripos($aiml_pattern, '<bot') !== false)
        {
            ...
            $btReplace = parse_bot_tag(new SimpleXMLElement($botTag));
 ...
 **/
function score_matches($allrows, $pattern)
{
    ...
    $topic = get_topic();
    $that = (isset ($_SESSION['convoArr']['that'][1][1])) ? $_SESSION['convoArr']['that'][1][1] : '';
    $default_pattern = $_SESSION['convoArr']['conversation']['default_aiml_pattern'];
    $bot_parent_id = $_SESSION['convoArr']['conversation']['bot_parent_id'];
...
*/
    ...
    $data = $bestResponse;
    $_SESSION['convoArr']['conversation']['aimlData'] = $data;
 ...
 * examples
 *
 * $_SESSION['convoArr']['conversation']['bot_id'] = $_SESSION['convoArr']['conversation']['bot_id']
 */
function get_convo_var($index_1, $index_2 = '~NULL~', $index_3 = 1, $index_4 = 1)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Get from ConvoArr [$index_1][$index_2][$index_3][$index_4]", 4);

    if ((isset ($_SESSION['convoArr'][$index_1])) && (!is_array($_SESSION['convoArr'][$index_1])) && ($_SESSION['convoArr'][$index_1] != ''))
    {
        $value = $_SESSION['convoArr'][$index_1];
    }
    elseif ((isset ($_SESSION['convoArr'][$index_1][$index_3])) && (!is_array($_SESSION['convoArr'][$index_1][$index_3])) && ($_SESSION['convoArr'][$index_1][$index_3] != ''))
    {
        $value = $_SESSION['convoArr'][$index_1][$index_3];
    }
    elseif ((isset ($_SESSION['convoArr'][$index_1][$index_3][$index_4])) && (!is_array($_SESSION['convoArr'][$index_1][$index_3][$index_4])) && ($_SESSION['convoArr'][$index_1][$index_3][$index_4] != ''))
    {
        $value = $_SESSION['convoArr'][$index_1][$index_3][$index_4];
    }
    elseif ((isset ($_SESSION['convoArr'][$index_1][$index_2])) && (!is_array($_SESSION['convoArr'][$index_1][$index_2])) && ($_SESSION['convoArr'][$index_1][$index_2] != ''))
    {
        $value = $_SESSION['convoArr'][$index_1][$index_2];
    }
    elseif ((isset ($_SESSION['convoArr'][$index_1][$index_2][$index_3])) && (!is_array($_SESSION['convoArr'][$index_1][$index_2][$index_3])) && ($_SESSION['convoArr'][$index_1][$index_2][$index_3] != ''))
    {
        $value = $_SESSION['convoArr'][$index_1][$index_2][$index_3];
    }
    elseif ((isset ($_SESSION['convoArr'][$index_1][$index_2][$index_3][$index_4])) && (!is_array($_SESSION['convoArr'][$index_1][$index_2][$index_3][$index_4])) && ($_SESSION['convoArr'][$index_1][$index_2][$index_3][$index_4] != ''))
    {
        $value = $_SESSION['convoArr'][$index_1][$index_2][$index_3][$index_4];
    }
    else {
        $value = '';
    }
...
function get_client_property($name)
{
    ...
    if ($rowCount != 0)
    {
        ...
        $_SESSION['convoArr']['client_properties'][$name] = $response;
...
function find_userdefined_aiml()
{
    ...
    $bot_id = $_SESSION['convoArr']['conversation']['bot_id'];
    $bot_parent_id = $_SESSION['convoArr']['conversation']['bot_parent_id'];
    $c_id = $_SESSION['convoArr']['conversation']['convo_id']; //$_SESSION['convoArr']['conversation'][''];user_id
    $lookingfor = _strtoupper($_SESSION['convoArr']['aiml']['lookingfor']);
    $params = array(
        ':user_id' => $c_id,
        ':bot_id'  => $bot_id
    );
...
endSQL;
    ...
    $lastthat = (isset ($_SESSION['convoArr']['that'][1][1])) ? $_SESSION['convoArr']['that'][1][1] : '';
...
function get_aiml_to_parse()
{
    ...
    $lookingfor = $_SESSION['convoArr']['aiml']['lookingfor'];
    $current_thatpattern = (isset ($_SESSION['convoArr']['that'][1][1])) ? $_SESSION['convoArr']['that'][1][1] : '';
    $current_topic = get_topic();
    $default_aiml_pattern = _strtolower($_SESSION['convoArr']['conversation']['default_aiml_pattern']);
    $bot_parent_id = $_SESSION['convoArr']['conversation']['bot_parent_id'];
    $raw_that = (isset ($_SESSION['convoArr']['that'])) ? print_r($_SESSION['convoArr']['that'], true) : '';

    //check if match in user defined aiml
    $allrows = find_userdefined_aiml();

    //if there is no match in the user defined aiml table
    if ((empty($allrows)))
    {
        //look for a match in the normal aiml tbl
        $allrows = find_aiml_matches();
    }
    //unset all irrelvant matches
    $allrows = unset_all_bad_pattern_matches($allrows, $lookingfor);
    //score the relevant matches
    $allrows = score_matches($allrows, $lookingfor);
    //get the highest scoring row
    $WinningCategory = get_highest_scoring_row($allrows, $lookingfor);
    ...
    if (($curPattern == $default_aiml_pattern || $curPattern == '*') && _strtolower($lookingfor) != $default_aiml_pattern) //
    {
        $bot_id = $_SESSION['convoArr']['conversation']['bot_id'];
        $user_id = $_SESSION['convoArr']['conversation']['user_id'];
        $rawSay = $_SESSION['convoArr']['conversation']['rawSay'];
        addUnknownInput($rawSay, $bot_id, $user_id);
    ...
    $_SESSION['convoArr']['aiml']['pattern'] = $WinningCategory['pattern'];
    $_SESSION['convoArr']['aiml']['thatpattern'] = $WinningCategory['thatpattern'];
    $_SESSION['convoArr']['aiml']['template'] = $WinningCategory['template'];
    $_SESSION['convoArr']['aiml']['html_template'] = '';
    $_SESSION['convoArr']['aiml']['topic'] = $WinningCategory['topic'];
    $_SESSION['convoArr']['aiml']['score'] = (isset($WinningCategory['score'])) ? $WinningCategory['score'] : 0;
    $_SESSION['convoArr']['aiml']['aiml_id'] = (isset($WinningCategory['aiml_id'])) ? $WinningCategory['aiml_id'] : -1;
    //return
...
function find_aiml_matches()
{
    ...
    $default_aiml_pattern = $_SESSION['convoArr']['conversation']['default_aiml_pattern'];

    $_SESSION['convoArr']['aiml']['lookingfor'] = str_replace('  ', ' ', $_SESSION['convoArr']['aiml']['lookingfor']);
    $lookingfor = trim(_strtoupper($_SESSION['convoArr']['aiml']['lookingfor']));

    //get the stored topic
    $storedtopic = trim(_strtoupper(get_topic()));
...
endSQL;
    ...
    {
        runDebug(__FILE__, __FUNCTION__, __LINE__, 'Error: FOUND NO AIML matches in DB.', 1);
        addUnknownInput($lookingfor, $bot_id, $user_id);
        runDebug(__FILE__, __FUNCTION__, __LINE__, "Added input '{$lookingfor}' to the unknown_inputs table.", 1);
...
function buildSearchRegEx()
{
    ...
    $_SESSION['convoArr']['regExSearch'] = $regExSearch;
