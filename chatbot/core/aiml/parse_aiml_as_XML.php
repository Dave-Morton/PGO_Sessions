...
function parse_aiml_as_XML()
{
    ...
    $template = remove_comment_tags($_SESSION['convoArr']['aiml']['template']);
    ...
    $responseArray = parseTemplateRecursive($aimlTemplate);
    ...
    $_SESSION['convoArr']['aiml']['parsed_template'] = $botsay;
 ...
 * @return string
 */
function parseTemplateRecursive(SimpleXMLElement $element, $level = 0)
{
    ...
    if (function_exists($func))
    {
        ...
        if (!in_array(strtolower($parentName), $doNotParseChildren))
        {
            ...
            $retVal = $func($element, $parentName, $level);
    ...
    if ($children->count() > 0 && is_object($retVal))
    {
        foreach ($children as $child)
        {
            ...
            $tmpResponse = parseTemplateRecursive($child, $level + 1);
 ...
 * @return string
 */
function parse_text_tag($element, $parentName, $level)
{
 ...
 * @return array
 */
function parse_star_tag($element, $parentName, $level)
{
    ...
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Current AIML Array:\n" . print_r($_SESSION['convoArr']['aiml'], true), 4);
    runDebug(__FILE__, __FUNCTION__, __LINE__, "star index = $index.", 4);

    $star = isset($_SESSION['convoArr']['aiml']['stars'][(int)$index]) ? $_SESSION['convoArr']['aiml']['stars'][(int)$index] : '';
 ...
 * @return array
 */
function parse_thatstar_tag($element, $parentName, $level)
{
    ...
    $star = $_SESSION['convoArr']['aiml']['that_stars'][(int)$index];
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Adding '$star' to the response array.", 4);
    $response[] = $star;
 ...
 * @return array
 */
function parse_topicstar_tag($element, $parentName, $level)
{
    ...
    $star = $_SESSION['convoArr']['aiml']['topic_stars'][(int)$index];
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Adding '$star' to the response array.", 4);
    $response[] = $star;
 ...
 * @return string
 */
function parse_date_tag($element, $parentName, $level)
{
 ...
 * @return string
 */
function parse_random_tag($element, $parentName, $level)
{
    ...
    $response = parseTemplateRecursive($liArray[$pick], $level + 1);
 ...
 * @return string
 */
function parse_get_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Client properties(get): ' . print_r($_SESSION['convoArr']['client_properties'], true), 2);
    ...
    $stars = $_SESSION['convoArr']['aiml']['stars'];
    ...
    switch (true)
    {
        case (in_array($var_name, $cpKeys)):
            ...
            break;
        default: $out = get_client_property($var_name);

    }
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Client properties(get2): ' . print_r($_SESSION['convoArr']['client_properties'], true), 2);
    return $out;
}
 ...
 * @return string
 */
function parse_set_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing the SET tag.', 2);
    global $user_name, $remember_up_to, $bot_id;
    $var_value = tag_to_string($element, $parentName, $level, 'element');
    $var_name = (string)$element->attributes()->name;
    $var_name = ($var_name == '*') ? $_SESSION['convoArr']['aiml']['stars'][1] : $var_name;
    for ($n = 2; $n <= $remember_up_to; $n++) # index multiple star values
    {
        $var_name = ($var_name == "*$n") ? $_SESSION['convoArr']['aiml']['stars'][$n] : $var_name;
    }
    $user_id = $_SESSION['convoArr']['conversation']['user_id'];
    $_SESSION['convoArr']['client_properties'][$var_name] = $var_value;
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Adding '{$var_name}' to the client properties array. Value is '{$var_value}'", 2);
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Client properties: ' . print_r($_SESSION['convoArr']['client_properties'], true), 2);
 ...
 * @return string
 */
function parse_think_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'I\'m considering parsing a THINK tag.', 2);
    /** @noinspection PhpUnusedLocalVariableInspection */
    $response_string = tag_to_string($element, $parentName, $level, 'element');
 ...
 * @return string
 */
function parse_bot_tag($element)
{
    global $remember_up_to;
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing a BOT tag.', 2);

    $attributeName = (string)$element->attributes()->name;
    $attributeName = ($attributeName == '*') ? $_SESSION['convoArr']['aiml']['stars'][1] : $attributeName;

    for ($n = 2; $n <= $remember_up_to; $n++) # index multiple star values
    {
        $attributeName = ($attributeName == "*$n") ? $_SESSION['convoArr']['aiml']['stars'][$n] : $attributeName;
    }

    $response = (!empty ($_SESSION['convoArr']['bot_properties'][$attributeName])) ? $_SESSION['convoArr']['bot_properties'][$attributeName] : 'undefined';
 ...
 * @return string
 */
function parse_id_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing an ID tag.', 2);

    return $_SESSION['convoArr']['conversation']['convo_id'];
}
 ...
 * @return string
 */
function parse_version_tag($element, $parentName, $level)
{
 ...
 * @return string
 */
function parse_uppercase_tag($element, $parentName, $level)
{
 ...
 * @return string
 */
function parse_lowercase_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'parsing a lowercase tag.', 2);

    $response = tag_to_string($element, $parentName, $level, 'element');
 ...
 * @return string
 */
function parse_sentence_tag($element, $parentName, $level)
{
 ...
 * @return string
 */
function parse_formal_tag($element, $parentName, $level)
{
    global $charset;
 ...
 * @return string
 */
function parse_srai_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing an SRAI tag.', 2);
    $starArray = array('~<star[ ]?/>~i', '~<star index="\d+"[ ]?\/>~');
    $elementXML = $element->asXML();
    $srai = tag_to_string($element, $parentName, $level, 'element');
    runDebug(__FILE__, __FUNCTION__, __LINE__, "SRAI parsed to {$srai}.", 2);
    $_SESSION['convoArr']['aiml']['lookingfor'] = $srai;
    set_wildcards('srai');
    $response = run_srai($_SESSION['convoArr']['aiml']['lookingfor']);
 ...
 * @return string
 */
function parse_sr_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing an SR tag.', 4);
    $response = run_srai($_SESSION['convoArr']['aiml']['stars'][1]);
 ...
 * @return array|string
 */
function parse_condition_tag($element, $parentName, $level)
{
            ...
            array_key_exists('exists', $attributesArray)) // condition tag with either VALUE, CONTAINS or EXISTS attributes
    {
        ...
        //$test_value = trim(get_client_property($condition_name));
        $test_value = (isset($_SESSION['convoArr']['client_properties'][$condition_name])) ?
            $_SESSION['convoArr']['client_properties'][$condition_name] :
            'undefined';
    ...
    elseif (array_key_exists('name', $attributesArray)) // this ~SHOULD~ just trigger if the NAME value is present, and ~NOT~ NAME and (VALUE|CONTAINS|EXISTS)
    {
        ...
        $condition_name = (string)$element['name'];
        //$test_value = trim(get_client_property($condition_name));
        $test_value = isset($_SESSION['convoArr']['client_properties'][$condition_name]) ? $_SESSION['convoArr']['client_properties'][$condition_name] : 'undefined';
    ...
    if (!empty ($children))
    {
        foreach ($children as $child)
        {
            $response[] = parseTemplateRecursive($child, $level + 1);
        }
 ...
 * @return string
 */
function parse_person_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing a PERSON tag.', 2);

    $response_string = tag_to_string($element, $parentName, $level, 'star');
    $response = swapPerson(2, $response_string);
 ...
 * @return string
 */
function parse_person2_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing a PERSON2 tag.', 2);

    $response_string = tag_to_string($element, $parentName, $level, 'star');
    $response = swapPerson(3, $response_string);
 ...
 * @return string
 */
function parse_html_tag($element, $parentName, $level)
{
    ...
    foreach ($kids as $kid)
    {
        $response_string .= implode_recursive(' ', parseTemplateRecursive($kid, $level + 1), __FILE__, __FUNCTION__, __LINE__); //
    }
 ...
 * @return string
 */
function parse_gender_tag($element, $parentName, $level)
{
    ...
    $nounList = $_SESSION['convoArr']['nounList'];
 ...
 * @return string
 */
function parse_that_tag($element, $parentName, $level)
{
    ...
    if (!empty($element))
    {
        ...
        {
            $response = $_SESSION['convoArr']['raw_that'][(int)$index];
            //$index .= ',1';
        }
        ...
        if (strstr($index, ',') !== false)
        {
            ...
            $thatArray = $_SESSION['convoArr']['raw_that'];
            ...
            if (!empty($_SESSION['convoArr']['raw_that'][$index1][$index2]))
            {
                $response = $_SESSION['convoArr']['raw_that'][$index1][$index2];
            }
            else {
                $response = $_SESSION['convoArr']['raw_that'][$index1];
            }
        }
    ...
    {
        $response_string = implode_recursive(' ', $_SESSION['convoArr']['raw_that'][1], __FILE__, __FUNCTION__, __LINE__);
    }
 ...
 * @return string
 */
function parse_input_tag($element, $parentName, $level)
{
    ...
    $response_string = $_SESSION['convoArr']['input'][$input_index];
 ...
 * @return string
 */
function parse_system_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing a SYSTEM tag (May God have mercy on us all).', 2);

    $system_call = tag_to_string($element, $parentName, $level, 'element');
 ...
 * @return string
 */
function parse_learn_tag($element, $parentName, $level)
{
    ...
    $pattern2store = (!empty($patternEvalXpath)) ?
        quickParseEval($patternText, 'pattern', 0) :
    ...
    $thatpattern2store = (!empty($thatpatternEvalXpath)) ?
        quickParseEval($thatpatternText, 'that', 0) :
    ...
    $template2store = (!empty($templateEvalXpath)) ?
        quickParseEval($templateText, 'template', 0) :
 ...
 * @return string
 */
function parse_eval_tag($element, $parentName, $level)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, 'Parsing an EVAL tag.', 2);

    $responseArray = parseTemplateRecursive($element, $level);
 ...
 * @return string
 */
function tag_to_string($element, $parentName, $level, $type = 'element')
{
    ...
    else
    {
        switch ($type)
        {
            case 'element':
                ...
                break;
            default:
                $response[] = isset($_SESSION['convoArr']['aiml']['stars'][1]) ? $_SESSION['convoArr']['aiml']['stars'][1] : '';
        }
    }
    $response_string = implode_recursive(' ', $response, __FILE__, __FUNCTION__, __LINE__);
...
function quickParseEval($elementText, $parentName, $level = 0)
{
    $search = '~<eval>.*?</eval>~';
    ...
    foreach ($found as $match)
    {
        ...
        else
        {
            ...
            foreach ($curEval->children() as $childNode)
            {
                ...
                {
                    $curResponse[] = $curFunc($childNode, $curParent, 0);
                }
                else {
