 ...
 * @return string $tmp
 **/
function swapPerson($person, $input)
{
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Person:$person In:$input", 4);

    $name = $_SESSION['convoArr']['client_properties']['name'];
    $gender = (isset($_SESSION['convoArr']['client_properties']['gender'])) ? $_SESSION['convoArr']['client_properties']['gender'] : 'unknown';
    $tmp = trim($input);
 ...
 * @return void
 */
function parse_matched_aiml($type = "normal", $aiml_pattern = '')
{
    //which debug mode?
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Run the aiml parse in $type mode (normal or srai)", 3);
    //save_file(_LOG_PATH_ . __FUNCTION__ . '.' . 'convo_array.txt', print_r(true));
...
function save_for_nextturn()
{
    ...
    $savethis = $_SESSION['convoArr']['aiml']['parsed_template'];
    push_on_front_convoArr('that_raw', $savethis);
    push_on_front_convoArr('that', $savethis);
}
 ...
 * @return void
 */
function set_wildcards($type)
{
    if (!isset($_SESSION['convoArr']['aiml']['srai_input'])) {
        $_SESSION['convoArr']['aiml']['stars'] = array();
        $_SESSION['convoArr']['aiml']['that_stars'] = array();
        $_SESSION['convoArr']['aiml']['topic_stars'] = array();
    }
    // Set pattern wildcards
    $aiml_pattern = trim($_SESSION['convoArr']['aiml']['pattern']);
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Setting Wildcards. Pattern = '$aiml_pattern'", 2);
/* debugging code
    $aimlArr = $_SESSION['convoArr']['aiml'];
...
*/
    ...
    if ($ap != $aiml_pattern)
    {
        ...
        if (!isset ($_SESSION['convoArr']['aiml']['user_raw'])) $_SESSION['convoArr']['aiml']['user_raw'] = normalize_text($_SESSION['convoArr']['aiml']['lookingfor'], false);
        $checkAgainst = ($type == 'normal') ? $_SESSION['convoArr']['aiml']['user_raw'] : $_SESSION['convoArr']['aiml']['lookingfor'];
        $regEx = "~{$ap}$~siuU";
        ...
        if (preg_match_all($regEx, $checkAgainst, $matches))
        {
            ...
            for ($i = 1; $i < count($matches); $i++)
            {
                ...
                $_SESSION['convoArr']['aiml']['stars'][$i] = $curStar;
                runDebug(__FILE__, __FUNCTION__, __LINE__, 'Star array = ' . print_r($_SESSION['convoArr']['aiml']['stars'], true), 2);
            }
        }
        else {
    ...
    $aiml_thatpattern = trim($_SESSION['convoArr']['aiml']['thatpattern']);
        ...
        if ($tp != $aiml_thatpattern)
        {
            ...
            if (preg_match_all("~{$tp}$~siuU", $checkAgainst, $matches))
            {
                ...
                for ($i = 1; $i < count($matches); $i++)
                {
                    ...
                    $_SESSION['convoArr']['aiml']['that_stars'][$i] = $curStar;
                }
            }
            else {
                runDebug(__FILE__, __FUNCTION__, __LINE__, "Something is not right here.", 2);
    ...
    if (!empty($aiml_topic))
    {
        ...
        if ($topp != $aiml_topic)
        {
            ...
            $topic = $_SESSION['convoArr']['client_properties']['topic'];
            ...
            if (preg_match_all("~{$topp}$~siuU", $checkAgainst, $matches))
            {
                ...
                for ($i = 1; $i < count($matches); $i++)
                {
                    ...
                    $_SESSION['convoArr']['aiml']['topic_stars'][$i] = $curStar;
                }
            }
            else {
                $_SESSION['convoArr']['aiml']['topic_stars'][1] = '';
                runDebug(__FILE__, __FUNCTION__, __LINE__, "Something is not right here.", 2);
            }
        }
    }
 ...
 **/
function run_srai($now_look_for_this)
{
    global $srai_iterations, $error_response, $dbn;

    $currentStars = $_SESSION['convoArr']['aiml']['stars'];
    ...
    if ($bot_parent_id != 0 && $bot_parent_id != $bot_id)
    {
        ...
        if ($num_rows > 0)
        {
          ...
          if (!empty($row))
          {
            ...
            $responseArray = parseTemplateRecursive($sraiTemplate);
...
*/
    ...
    $allrows = unset_all_bad_pattern_matches($allrows, $now_look_for_this);
    ...
    $allrows = score_matches($allrows, $now_look_for_this);
    //get the highest
    $allrows = get_highest_scoring_row($allrows, $now_look_for_this);
    ...
    runDebug(__FILE__, __FUNCTION__, __LINE__, $_SESSION['convoArr']['aiml']['html_template'], 4);
    ...
    if ($srai_iterations > 10)
    {
        ...
        $_SESSION['convoArr']['aiml']['parsed_template'] = $error_response;

        return $error_response;
    }

    //$tmp_convoArr = array();
    $tmp_convoArr = $_SESSION['convoArr'];
    ...
    $tmp_convoArr['aiml']['thatpattern'] = $_SESSION['convoArr']['aiml']['thatpattern'];
    ...
    $_SESSION['convoArr']['client_properties'] = $tmp_convoArr['client_properties'];
    $srai_iterations--;
...
{
    if ((trim($item)) != (trim($_SESSION['convoArr']['stack']['top'])))
    {
        runDebug(__FILE__, __FUNCTION__, __LINE__, "Pushing $item onto to the stack", 4);

        $_SESSION['convoArr']['stack']['last'] = $_SESSION['convoArr']['stack']['seventh'];
        $_SESSION['convoArr']['stack']['seventh'] = $_SESSION['convoArr']['stack']['sixth'];
        $_SESSION['convoArr']['stack']['sixth'] = $_SESSION['convoArr']['stack']['fifth'];
        $_SESSION['convoArr']['stack']['fifth'] = $_SESSION['convoArr']['stack']['fourth'];
        $_SESSION['convoArr']['stack']['fourth'] = $_SESSION['convoArr']['stack']['third'];
        $_SESSION['convoArr']['stack']['third'] = $_SESSION['convoArr']['stack']['second'];
        $_SESSION['convoArr']['stack']['second'] = $_SESSION['convoArr']['stack']['top'];
        $_SESSION['convoArr']['stack']['top'] = $item;
    }
    else {
...
function pop_stack()
{
    $item = trim($_SESSION['convoArr']['stack']['top']);
    runDebug(__FILE__, __FUNCTION__, __LINE__, "Popped $item off the stack", 4);

    $_SESSION['convoArr']['stack']['top'] = $_SESSION['convoArr']['stack']['second'];
    $_SESSION['convoArr']['stack']['second'] = $_SESSION['convoArr']['stack']['third'];
    $_SESSION['convoArr']['stack']['third'] = $_SESSION['convoArr']['stack']['fourth'];
    $_SESSION['convoArr']['stack']['fourth'] = $_SESSION['convoArr']['stack']['fifth'];
    $_SESSION['convoArr']['stack']['fifth'] = $_SESSION['convoArr']['stack']['sixth'];
    $_SESSION['convoArr']['stack']['sixth'] = $_SESSION['convoArr']['stack']['seventh'];
    $_SESSION['convoArr']['stack']['seventh'] = $_SESSION['convoArr']['stack']['last'];
    $_SESSION['convoArr']['stack']['last'] = "om";
 ...
 **/
function make_learn($pattern, $template)
{
    global $dbn;
