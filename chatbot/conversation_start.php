...
if (!empty($say)) {
    ...
    if ($lc_say == 'clear properties' || $lc_say == ':reset bot')
    {
        ...
        $_SESSION = array();
        $user_id = (isset($_SESSION['convoArr']['conversation']['user_id'])) ? $_SESSION['convoArr']['conversation']['user_id'] : -1;
        ...
        $_SESSION['convoArr']['client_properties'] = null;
        ...
        if ($row !== false) {
            $user_id = $row['id'];
            $_SESSION['convoArr']['conversation']['user_id'] = $user_id;
            $_SESSION['convoArr']['conversation']['convo_id'] = $new_convo_id;
    ...
    run_pre_input_addons($say);
    $say = $_SESSION['convoArr']['say'];
    $rawSay = $say;
    $_SESSION['convoArr']['conversation']['rawSay'] = $rawSay;
    ...
    {
        $_SESSION['convoArr']['conversation']['user_name'] = $user_name = $form_vars['name'];
    }
    if (!isset($_SESSION['convoArr']['conversation']['user_id']) && isset($user_id))
    {
        $_SESSION['convoArr']['conversation']['user_id'] = $user_id;
    }
    ...
    check_set_format();
    load_that();
    //save_file(_LOG_PATH_ . 'ca.txt', print_r(,true));
    buildNounList();
    $_SESSION['convoArr']['time_start'] = $time_start;
    load_bot_config();
...
handleDebug($time); // Make sure this is the last line in the file, so that all debug entries are captured.
pgo_session_gc();